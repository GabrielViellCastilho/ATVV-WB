...
export default function FormularioCadastroProduto({ tema }: Props) {
  ...
  const criarProduto = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:3069/produtos", {
        method: "POST",
  ...
  const deletarProduto = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3069/produtosPorNome/${nomeRemover}`, {
        method: "DELETE",
      });
  ...
  const abrirModalAtualizar = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3069/produtos/${nomeAntigo}`);
  ...
  const atualizarProduto = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3069/produtosPorNome/${nomeAntigo}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="row">
      {/* Criar Produto */}
      <form className="col s10 offset-s1" onSubmit={criarProduto}>
        <h4 className="deep-purple-text text-darken-2">Criar Produto</h4>
        <div className="row">
      ...
      {/* Remover Produto */}
      <form className="col s10 offset-s1" onSubmit={deletarProduto}>
        <h4 className="deep-purple-text text-darken-2">Remover Produto</h4>
        <div className="row">
          <div className="input-field col s12 m6">
      ...
      <form className="col s10 offset-s1">
        <h4 className="deep-purple-text text-darken-2">Atualizar Produto</h4>
        <div className="row">
          <div className="input-field col s12 m6">
      ...
      <div id="modalProduto" className="modal">
        <div className="modal-content">
          <form className="col s12" onSubmit={atualizarProduto}>
            <h4 className="blue-text text-darken-2">Atualizar Produto</h4>
            <div className="row">
              <div className="input-field col s12 m6">
        ...
        <div className="modal-footer">
          <a className="modal-close waves-effect waves-green btn-flat blue-text text-darken-2">
            Sair
          </a>
        </div>
      </div>
