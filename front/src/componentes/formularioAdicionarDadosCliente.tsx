...
const FormSelector = ({ cpf, tema }: Props) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:3069/clientes/cpf/${cpf}`);
            ...
            switch (selectedOption) {
                ...
                case "Produto Consumido":
                    const resProduto = await fetch(`http://localhost:3069/produtos/${produto}`);
                ...
                case "Serviço Consumido":
                    const resServico = await fetch(`http://localhost:3069/servicos/${servico}`);
            ...
            // Enviar os dados
            const response = await fetch(`http://localhost:3069${endpoint}`, {
                method: "POST",
        ...
        <div>
            ...
            {selectedOption === "Telefone" && (
                <>
                    <div className="input-field col s2">
                        ...
                        <label className="active deep-purple-text text-lighten-2">DDD</label>
                    </div>
                    <div className="input-field col s10">
                        <input type="text" value={telefone.numero} onChange={e => setTelefone({ ...telefone, numero: e.target.value })} />
                        <label className="active deep-purple-text text-lighten-2">Telefone</label>
                    </div>
                </>
            )}
            ...
            {selectedOption === "RG" && (
                <>
                    <div className="input-field col s6">
                        ...
                        <label className="active deep-purple-text text-lighten-2">RG</label>
                    </div>
                    <div className="input-field col s6">
                        <input type="date" value={rg.dataEmissao} onChange={e => setRG({ ...rg, dataEmissao: e.target.value })} />
                        <label className="active deep-purple-text text-lighten-2">Data de Emissão</label>
                    </div>
                </>
            )}
            ...
            {selectedOption === "Produto Consumido" && (
                <div className="input-field col s12">
                    ...
                    <label className="active deep-purple-text text-lighten-2">Nome do Produto</label>
                </div>
            )}
            ...
            {selectedOption === "Serviço Consumido" && (
                <div className="input-field col s12">
                    ...
                    <label className="active deep-purple-text text-lighten-2">Nome do Serviço</label>
                </div>
            )}
